...
export const putContextData = async (data: ProjectContext) => {
  const projectId = `PROJECT#${data.projectTitle.replace(/\s+/g, '-').toLowerCase()}`; // Ensure uniqueness and facilitate querying
...
export const getContextData = async (projectTitle: string, userId: string): Promise<ProjectContext | null> => {

  console.log('projectTitle', projectTitle);
  const projectPk = `USER#${userId}`;
  const projectSk = `METADATA#PROJECT#${projectTitle.replace(/\s+/g, '-').toLowerCase()}`;

  console.log('project pk and sk', projectPk, projectSk)

  try {
    // Query the project context
    const projectResult = await dynamoDb.send(new QueryCommand({
      TableName: 'FeatureFlowContextTable',
    ...
    const projectContext: ProjectContext = projectResult.Items[0] as ProjectContext;

    // Optionally, query the steps for the project
    const stepsResult = await dynamoDb.send(new QueryCommand({
      TableName: 'FeatureFlowContextTable',
